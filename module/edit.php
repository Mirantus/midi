<?php
	//initialization
	define ('PAGE', 'module_edit');
	require_once('../inc/init.inc'); 
	require_once('init.inc');

	$id = $title = $text = $price = $image = $file = $name = $phone = $icq = $url = $email = $city = $zip = $address = $company = $occupation = $user = $ip = $date = $access = $rate = $alert = '';
	$result = true;
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		if ($result)
		{
			if ($mode == 'add')
			{		
				sql_execute('INSERT IGNORE INTO `' . MODULE . '` (title, text, price, name, phone, icq, url, email, city, zip, address, company, occupation, user, ip, date, access, rate) VALUES ("' . $title . '", "' . $text . '", "' . $price . '", "' . $name . '", "' . $phone . '", "' . $icq . '", "' . $url . '", "' . $email . '", "' . $city . '", "' . $zip . '", "' . $address . '", "' . $company . '", "' . $occupation . '", "' . $user . '", "' . $ip . '", "' . $date . '", "' . $access . '", "' . $rate . '");');			
				$id = mysql_insert_id();
			...
			{			
				...
				sql_execute('UPDATE `' . MODULE . '` SET title = "' . $title . '", text = "' . $text . '", price = "' . $price . '", ' . $image . 'name = "' . $name . '", phone = "' . $phone . '", icq = "' . $icq . '", url = "' . $url . '", email = "' . $email . '", city = "' . $city . '", zip = "' . $zip . '", address = "' . $address . '", company = "' . $company . '", occupation = "' . $occupation . '", user = "' . $user . '", ip = "' . $ip . '", date = "' . $date . '", access = "' . $access . '", rate = "' . $rate . '" WHERE id = ' . $id . ';');
			}		
